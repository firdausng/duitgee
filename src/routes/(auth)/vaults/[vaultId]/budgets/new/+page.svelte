<script lang="ts">
	...
	const { form, errors, enhance, delayed } = superForm(data.form, {
		validators: valibotClient(createBudgetSchema),
		SPA: true,
		async onUpdate({ form }) {
			if (!form.valid) {
			...
			try {
				...
				const response = await ofetch('/api/createBudget', {
					method: 'POST',
					body: payload,
					headers: {
				...
				if (response.success === false) {
					toast.error(response.error || 'Failed to create budget');
					return;
				}
				...
				console.error({
					...error,
					message: '[budget:new:action] Failed to create budget'
				});
				const errorMessage = error?.data?.error || error?.message || 'Failed to create budget. Please try again.';
	...
	if (!$form.startDate) {
		...
		$form.startDate = formatDatetimeLocal(startOfMonth);
	}
...
<svelte:head>
	<title>Create Budget - DuitGee</title>
</svelte:head>
...
<div class="relative min-h-screen flex flex-col">
	<div class="container mx-auto py-4 md:py-8 px-4 flex-1">
		...
		<div class="mb-4 md:mb-6">
			...
			<p class="text-sm text-muted-foreground mt-1">Set spending limits for better financial control</p>
		</div>
		...
		<Card>
			<CardHeader>
				...
				<CardDescription>Configure your spending limit and tracking preferences</CardDescription>
			</CardHeader>
			<CardContent>
...
<!--							</div>-->
					...
					<div class="flex items-center space-x-2">
						<input
							...
							class="h-4 w-4 rounded border-gray-300"
						/>
						<Label for="isActive" class="cursor-pointer">Activate immediately</Label>
					</div>
				</form>
			</CardContent>
		</Card>
