...
<div class="container mx-auto py-6 px-4 max-w-2xl">
	...
	<!-- Income-deduction lock — this expense was generated by an income entry. -->
	{#if data.expense.incomeEntryId}
		<Card class="mb-4 border-amber-300/60 dark:border-amber-700/60 bg-amber-50/40 dark:bg-amber-950/20">
			<CardContent class="py-4 space-y-2">
				<p class="text-sm">
					🔒 <span class="font-medium">This expense was generated from an income entry.</span>
					Edits go through the income page so the breakdown snapshot stays consistent.
				</p>
				<Button
					type="button"
					size="sm"
					onclick={() => goto(`/vaults/${data.vaultId}/income/${data.expense.incomeEntryId}/edit`)}
				>
					Open income entry →
				</Button>
			</CardContent>
	...
	{#if isLoading || isDeleting}
		<Spinner />
	{:else if !data.expense.incomeEntryId}
		<Card>
