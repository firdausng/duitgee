		...
		<CardContent class="py-3">
			<div class="flex items-center justify-between mb-2">
				<div class="flex items-center gap-2">
					...
					<h3 class="text-xs font-semibold">Track Budget</h3>
				</div>
				{#if selectedBudgetId}
					<Button
						variant="ghost"
			...
			<div class="flex flex-wrap gap-2">
				{#each budgets as budget (budget.id)}
					<button
						...
						class="inline-flex flex-col items-start px-3 py-2 rounded-md border text-xs transition-colors {budget.id === selectedBudgetId ? 'bg-primary text-primary-foreground border-primary' : 'bg-background hover:bg-accent border-border'}"
					>
