<script lang="ts">
    ...
    const { form, errors, enhance, delayed } = superForm(data.form, {
        validators: valibotClient(deductFundSchema),
        SPA: true,
        async onUpdate({ form }) {
            if (!form.valid) {
            ...
            isLoading = true;
            try {
                const response = await ofetch('/api/deductFund', {
                    method: 'POST',
                    body: form.data,
                ...
                if (response.success === false) {
                    toast.error(response.error || 'Failed to deduct from fund');
                    return;
                }
                ...
                const msg = error?.data?.error || error?.message || 'Failed to deduct from fund. Please try again.';
                toast.error(msg);
            } finally {
                isLoading = false;
            }
...
<svelte:head>
    <title>Deduct Fund - DuitGee</title>
</svelte:head>
...
<div class="container mx-auto py-6 px-4">
    ...
    <Card>
        <CardHeader>
            <CardTitle>Remove Money from Fund</CardTitle>
        </CardHeader>
        <CardContent>
            <form method="POST" use:enhance class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <Input
                        ...
                        disabled={$delayed}
                        placeholder="Optional note for this deduction"
                    />
                </div>
