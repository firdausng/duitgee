							...
							<div class="p-4 space-y-4 max-h-[calc(80vh-200px)] overflow-y-auto">
								...
								<nav class="space-y-1">
									...
									{#if navVaultId()}
										...
										<button
											onclick={() => navigateAndClose(`/vaults/${navVaultId()}/calendar`)}
											class="w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm hover:bg-accent transition-colors {isActive(`/vaults/${navVaultId()}/calendar`) ? 'bg-accent' : ''}"
										>
											<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
