...
export const expenses = sqliteTable('expenses', {
    ...
    // Soft ref to incomeEntries.id. Set when this expense was auto-generated
    // as a salary-deduction child of an income entry. updateExpense and
    // deleteExpense reject mutations on rows with this set — edits must go
    // through the income entry to preserve the snapshot/expense coupling.
    ...
    incomeEntryIdx: index('idx_expenses_income_entry').on(table.incomeEntryId),
...
export const incomeTemplates = sqliteTable('income_templates', {
    ...
    // Salary breakdown defaults — JSON arrays of AllowanceLine / DeductionLine.
    // Allowances bump the income amount when entries seed from this template;
    // deductions auto-generate linked expense rows. See valibot for the shape.
...
export const incomeEntries = sqliteTable('income_entries', {
    ...
    // `amount` is the canonical total income (= baseAmount + sum of computed
    // allowance amounts, when a breakdown is captured). Stats keep using this.
    amount: real('amount').notNull(),
    // When the user itemizes base + allowances, baseAmount holds the pre-
    // allowance figure. Null = no breakdown captured; amount stands alone.
    baseAmount: real('base_amount'),
    // JSON snapshots of the breakdown lines + their resolved computedAmount at
    // entry-create time. Snapshot acts as both audit trail and the bridge for
    // cascade-edit (diff old vs new to mutate the right linked expense rows).
    allowances: text('allowances'),
    deductions: text('deductions'),
    date: text('date').notNull().$defaultFn(() => formatISO(new UTCDate())),
    paidTo: text('paid_to'), // Vault member ID; null = vault-level (household pool)
    note: text('note'),
    // Fund routing — when set, the create handler emits a `top_up` fundTransactions row
    // and stores its id in fundTransactionId for reversal on delete.
    fundId: text('fund_id'), // Soft ref to funds.id
    fundTransactionId: text('fund_transaction_id'), // Soft ref to fundTransactions.id
    // Provenance — set when this entry was generated by a recurring income rule (Slice 3).
    recurringIncomeId: text('recurring_income_id'), // Soft ref to recurringIncome.id (future)
    // CSV import provenance — reserved for the future income import flow.
    importBatchId: text('import_batch_id'),
    // Audit fields
    createdAt: text('created_at').$defaultFn(() => formatISO(new UTCDate())),
    createdBy: text('created_by').notNull(),
