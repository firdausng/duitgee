<script lang="ts" module>
    export interface ExpandableFabTemplate {
        id: string;
        name: string;
        icon: string | null;
        usageCount: number;
    ...
    export interface ExpandableFabProps {
        ...
        browseHref: string;
        /** When true the FAB renders but is disabled. */
        disabled?: boolean;
        class?: string;
    }
</script>
...
<script lang="ts">
    ...
    function toggle() {
        if (disabled) return;
        if (!hasTemplates) {
            // No templates — skip the popover entirely and go straight to scratch.
    ...
    function onDocumentClick(e: MouseEvent) {
        ...
        if (fabRef?.contains(target)) return;
        close();
    }
...
<div
    class={cn(
        'fixed right-6 z-50 bottom-[calc(1.5rem+env(safe-area-inset-bottom))] sm:bottom-6',
        className,
    )}
>
    <!-- Popover panel -->
    {#if open && hasTemplates}
        <div
            ...
            role="menu"
            class="absolute bottom-full right-0 mb-3 w-72 rounded-[var(--radius-md)] border bg-popover text-popover-foreground shadow-lg overflow-hidden"
            style="animation: fab-pop-in 150ms var(--ease-out);"
        >
    ...
    <button
        ...
        class={cn(
            ...
            'px-6 py-4 sm:px-5 sm:py-3.5 md:px-6 md:py-4',
            'font-semibold',
    ...
    >
        {#if open}
            <ChevronUp class="size-5 transition-transform duration-200" style="transform: rotate(180deg);" />
        {:else}
            <Plus class="size-5" />
        {/if}
